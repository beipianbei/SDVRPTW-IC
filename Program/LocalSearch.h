...
private:
	...
	int Delta;									// Total number of moves (RI and SWAP*) applied during the local search. Attention: this is not only a simple counter, it is also used to avoid repeating move evaluations
	std::vector < int > orderNodes;				// Randomized order for checking the nodes in the RI local search
	std::vector < int > RouteCliSeqU;			// 路由客户序列（有顺序）20241201
	std::vector < int > RouteCliSeqV;			// 路由客户序列（有顺序）20241201
	std::vector < int > addCliInfo;				// 路由客户序列（有顺序）20241201
	...
	Route * routeV ;
	Move * move;
	int nodeUPrevIndex, nodeUIndex, nodeXIndex, nodeXNextIndex, nodeUN;	
	int nodeVPrevIndex, nodeVIndex, nodeYIndex, nodeYNextIndex, nodeVN;
	...
	int TimeWindowIsMetCVRP(std::vector<int>& RouteCliSeq); //// 判断移动节点后续节点时间窗是否被满足20241201
	int TimeWindowIsMetswapStar(Node* U, Node* V); //// 判断移动节点后续节点时间窗是否被满足20241201
	...
	inline double penaltyExcessDuration(double myDuration) { 
		double excess = std::max<double>(0., myDuration - params.durationLimit);
		return excess * (1 + excess/1000); // 轻微超时惩罚低，严重超时惩罚剧增
	}
	inline double penaltyExcessLoad(double myLoad) { return std::max<double>(0., myLoad - params.vehicleCapacity) * penaltyCapacityLS; }
	void clientsPush_back(int a) { clients.push_back(clients[a]); } // 赋值一个客户
	...
	/*20250929 快速合规检查：把 node 插到目标路线是否安全 */
	static bool canInsert(int node, Route* tgtRoute);
	bool checkroute(Route* tgtRouteU, Route* tgtRouteV);
